...
export default function Testimonials() {
  ...
  // Pull translated testimonial content; fall back to constants for initials (not translated)
      ...
      <View style={IS_WEB ? { maxWidth: MAX_W, width: '100%', alignSelf: 'center' } : {}}>
        ...
        <Animated.View
          style={{
            ...
            backgroundColor: isDark ? C.darkCard : '#f0f9ff',
        ...
        >
          ...
          <Text style={{ color: isDark ? '#e2e8f0' : C.heading, fontFamily: F.regular, fontSize: IS_WEB ? 17 : 15, lineHeight: IS_WEB ? 30 : 26, fontStyle: 'italic', marginBottom: 28 }}>
            "{active.text}"
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
            <View style={{ width: 54, height: 54, borderRadius: 27, backgroundColor: AVATAR_COLORS[activeIdx % AVATAR_COLORS.length], alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: '#ffffff', shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 6, elevation: 3 }}>
              <Text style={{ color: '#ffffff', fontFamily: F.bold, fontSize: 16 }}>{activeInitials}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ color: C.heading, fontFamily: F.bold, fontSize: 16 }}>{active.name}</Text>
              <Text style={{ color: C.muted, fontFamily: F.regular, fontSize: 13, marginTop: 2 }}>{active.role}</Text>
            </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 8, marginBottom: IS_WEB ? 32 : 0 }}>
          {TESTIMONIALS.map((_, i) => (
            <TouchableOpacity key={i} onPress={() => changeTo(i)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
              <Animated.View style={{ height: 8, width: i === activeIdx ? 28 : 8, borderRadius: 4, backgroundColor: i === activeIdx ? C.yellow : '#d1d5db' }} />
            </TouchableOpacity>
          ))}
        </View>
          ...
          <View style={{ flexDirection: 'row', gap: 16, marginTop: 8 }}>
            {tItems.map((item, i) => (
              <TouchableOpacity key={item.name} onPress={() => changeTo(i)} style={{ flex: 1 }} activeOpacity={0.82}>
                <View style={{ borderRadius: 16, padding: 18, borderWidth: 2, borderColor: i === activeIdx ? C.yellow : '#e5e7eb', backgroundColor: i === activeIdx ? '#f0f9ff' : '#ffffff' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    ...
                    <View>
                      <Text style={{ color: C.heading, fontFamily: F.bold, fontSize: 13 }}>{item.name}</Text>
                      <Text style={{ color: C.muted, fontFamily: F.regular, fontSize: 11 }}>{item.role}</Text>
                    </View>
                  </View>
                  <Text style={{ color: C.muted, fontFamily: F.regular, fontSize: 12, lineHeight: 18, fontStyle: 'italic' }} numberOfLines={3}>
                    "{item.text}"
                  </Text>
                </View>
              </TouchableOpacity>
